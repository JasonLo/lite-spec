...
10. **Seed agent prompt files** for every `[test: agent:<path>]` citation. For each citation whose target lives under `specs/INTENT/I-<N_new>-<slug>/checks/`, `mkdir -p` the parent directory and, **only if the prompt file does not yet exist**, create it pre-populated with the SHALL line under a `## SHALL` heading. Read [`AGENT_PROMPT_SEED.template.md`](AGENT_PROMPT_SEED.template.md) (sibling of this `SKILL.md`) for the seed body. Substitute the one-line summary from elicitation, then write the file.
