...
Legacy untagged decision lines (pre-tag-rule entries that `spec-decisions` documents as valid on read) have no tag to validate and are naturally skipped. Likewise `[intent: none]` tags — project-level decisions per `spec-decisions` — carry no folder reference by design; the `\[intent: I-[0-9]+\]` pattern does not match them, so they are correctly skipped, never flagged as dangling.
