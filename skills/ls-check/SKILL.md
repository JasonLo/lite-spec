...
Verification is mechanical: each SHALL is checked individually, not vibe-checked as a whole. When an outcome carries a `[test: <runner>:<target>]` citation, `ls-check` executes it. Two flavors of runner exist:

- **Process runners** (`pytest`, `vitest`, `jest`, `cargo`, `go`, `npm`, `shell`) — `ls-check` invokes them via Bash; exit code 0 is the only path to a test-backed `pass`.
- **Agent runner** (`agent:<path-to-prompt-file>`) — `ls-check` spawns a subagent, hands it the prompt file plus the EARS line plus scope hints, parses a structured verdict block, and reports `pass`, `fail`, or `unverifiable`. Use this only for SHALLs that genuinely can't be checked by a normal test (UX claims, doc structure, narrative consistency).

Outcomes without any `[test: ...]` citation are classified `unverifiable` — there is no grep + LLM inline fallback. To drive a SHALL to `pass`, the user adds a citation via `/ls-intent refine`.
...
4. **For each `O-N`, classify code-drift.** The path is determined by whether `O-N` carries any `[test: ...]` citation. Pick exactly one:
   ...
   **No citation at all** — classify the outcome `unverifiable (no test citation)`. Emit a self-critique flag in the report: `O-N has no [test: ...] citation — re-invoke /ls-intent refine to add one (process runner, or agent:<prompt-path> for SHALLs that can't be checked programmatically).` This is the only nudge ls-check sends back toward ls-intent for citation coverage; the goal is the by-test + by-agent ratio climbing over time.
...
- [?] O-5: <EARS text> — unverifiable (no test citation).
      Flag: add a [test: ...] citation via /ls-intent refine.
...
The `Next:` line is **conditional** and follows the Handoff convention documented in `ls-init`. Emit it **only** when at least one outcome in the run was classified `unverifiable` for a reason that `/ls-intent refine` can fix (missing citation, vague EARS, unknown runner, whole-suite citation, constitution-forbidden runner, agent prompt empty, agent prompt path outside repo, malformed agent citation, agent reply malformed, agent invocation error). Pick the lowest-numbered affected `I-N` **from the intents in this run's scope** — if invoked with `--intent I-K`, the affected intent is always `I-K` (it is the only intent in scope), regardless of unverifiable outcomes that may exist in other intents from prior runs. **Do NOT** emit `Next:` for `unverifiable (agent skipped)`, `unverifiable (--no-tests)`, `fail` outcomes, `intent-ahead` drift, or constitution-principle failures — the first two are user-opted run-mode artifacts (the user passed the flag), the rest resolve in code or via `/ls-constitution amend` and `ls-check` can't tell which; the user's judgment owns the next move. When the run is clean (zero fail, zero unverifiable), omit `Next:` entirely — silence is the terminal signal.
...
For each agent citation, ls-check builds the subagent's prompt by concatenating these segments in order:

```
You are an EARS verification subagent for the lite-spec ls-check skill. Your job is to verify whether the implementation satisfies one specific EARS SHALL statement, using ONLY the criteria in the check prompt below. Return a structured verdict.
...
## Scope hints
Repo root: <cwd>
Code surface ls-check searched for this intent (you may inspect any of these):
- <path-1>
- <path-2>
- ...
...
Emit, as the LAST block of your reply, a fenced block tagged `ls-check-verdict` containing valid JSON with this exact schema:
...
Reason briefly before the verdict block if you want — you have one shot, no retries. The fenced block is parsed by ls-check; everything before it is discarded.
```

### Verdict parser

After invocation, ls-check extracts the **last** `ls-check-verdict` fenced block from the reply, parses it as JSON, and validates against this schema:
...
- **Legacy intent.md without `verdict_outcomes_passed_by_agent`** — treat the missing field as if it were `null`. ls-check adds it on the first run that flips the file (subject to the no-op short-circuit in step 9).
...
- NEVER cache agent results across runs. Every `/ls-check` invocation re-spawns every agent citation in scope. A cache would silently mask drift.
